...
import { getWithdrawList, getWithdrawDetail} from '@/api/api'
...
class PutList extends Component {
 ...
 componentDidMount() {
  ...
  loadList = () => {
    ...
    getWithdrawList(params).then(rs => {
      if (rs.success) {
  ...
  viewDataEvent = (item) => {
    getWithdrawDetail({orderNo: item.orderNo, token: this.state.token}).then(rs => {
      if (rs.success) {
  ...
  render() {
    ...
    const content = (
      <ul className={style['detailBox']}>
        <li className="bold g-tc">提现详情</li>
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (record) => (
          <div className="g-tc">{record === undefined ? '--' : record}</div>
        )
      },
      {
        title: '提现状态',
    ...
    return (
      <div className={style.arnings}>
        <ul className={style.search}>
          <li>
            <label className="mr10">申请时间</label>
            <RangePicker 
              className={`w2600`}
          ...
          <li>
            <label>提现单号</label>
            <Input className={style['ipt']} value={search.orderNo} onChange={e => this.changeFormEvent(e, 'orderNo')} /></li>
          <li>
            <label className="mr10">状态</label>
            <Select value={search.orderStatus} className="w180" onChange={e => this.changeFormEvent(e, 'orderStatus')}>
